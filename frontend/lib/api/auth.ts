...
// ログイン API
export const loginApi = async (credentials: LoginRequest): Promise<AuthToken> => {
  try {
    ...
    throw new Error(error.response?.data?.detail || error.message || 'ログインに失敗しました')
  }
}
...
// パスワードリセット要求 API
export const requestPasswordResetApi = async (email: string): Promise<{ message: string; email: string }> => {
  try {
    const response = await apiClient.post('/api/v1/auth/password-reset', { email })
    return response.data
  } catch (error: any) {
    throw new Error(error.response?.data?.detail || 'パスワードリセットの送信に失敗しました')
  }
}

// パスワードリセット確定 API
export const confirmPasswordResetApi = async (token: string, newPassword: string): Promise<{ message: string; email: string }> => {
  try {
    ...
    throw new Error(error.response?.data?.detail || 'パスワードリセットの確定に失敗しました')
  }
}
...
export const updateUserProfileApi = async (userData: Partial<User>): Promise<User> => {
  try {
    ...
    throw new Error(error.response?.data?.detail || 'ユーザー情報の更新に失敗しました')
  }
}
