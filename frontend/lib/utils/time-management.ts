 ...
 * - 営業時間外の場合は翌日の同じ時間に調整
 */
export function getDefaultReturnDateTime(
  pickupDateTime: Date,
  durationHours: number = DEFAULT_RENTAL_DURATION_HOURS,
): Date {
  const returnTime = new Date(
    pickupDateTime.getTime() + durationHours * 60 * 60 * 1000,
  );
...
export function calculateReturnDateTimeFromPickup(
  ...
  const returnDateTime = getDefaultReturnDateTime(
    pickupDateTime,
    durationHours,
  );
...
export function validateSearchConditions(
  ...
  // 最小レンタル時間（1時間）チェック
  const durationHours = calculateRentalDurationHours(
    pickupDateTime,
    returnDateTime,
  );
  if (durationHours < 1) {
