...
import { reservationApi, Reservation, GetReservationsParams } from '@/lib/api/reservations'
...
export function useReservations(params?: GetReservationsParams) {
  ...
  const updateReservationPaymentStatus = (reservationId: string, paymentStatus: 'pending' | 'completed' | 'failed', paymentMethod?: string, paymentReference?: string) => {
    ...
    console.log("予約一覧の支払いステータスを更新:", reservationId, paymentStatus);
  };
...
export function useReservation(id: string | null) {
  ...
  const updatePaymentStatus = (paymentStatus: 'pending' | 'completed' | 'failed', paymentMethod?: string, paymentReference?: string) => {
    if (reservation) {
