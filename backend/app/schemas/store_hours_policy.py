...
class DayHours(BaseModel):
    ...
    def validate_time_format(cls, v):
        ...
        try:
            ...
            raise ValueError(f"無効な時間フォーマットです: {v}. HH:MM形式で入力してください")
...
class CancellationPolicy(BaseModel):
    """キャンセルポリシースキーマ"""
    free_cancellation_hours: int = Field(..., ge=0, description="無料キャンセル可能時間（時間）")
    cancellation_fee_percentage: float = Field(..., ge=0, le=100, description="キャンセル料率（%）")
    no_show_fee_percentage: float = Field(..., ge=0, le=100, description="ノーショー料率（%）")
...
class PricingPolicy(BaseModel):
    ...
    late_return_fee_per_hour: float = Field(..., ge=0, description="延滞料（時間あたり）")
...
class LicenseRequirement(BaseModel):
    ...
    validity_period_months: int = Field(..., ge=1, le=24, description="免許有効期間（月）")
    international_license_accepted: bool = Field(..., description="国際免許受け入れ")


class StorePolicy(BaseModel):
    """店舗ポリシースキーマ"""
    cancellation_policy: CancellationPolicy = Field(..., description="キャンセルポリシー")
