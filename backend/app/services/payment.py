...
class PaymentService:
    ...
    def process_payment(
        self, reservation_id: str, payment_data: PaymentRequest, customer_id: str, idempotency_key: Optional[str] = None
    ) -> PaymentResponse:
        """
        決済処理を実行する
        ...
        if idempotency_key:
            ...
            if existing_payment:
                ...
                return PaymentResponse(
                    ...
                    failure_reason="カードが拒否されました" if existing_payment.payment_status == "failed" else None
                )
        ...
        if payment_data.card_token == "tok_failed_card":
            ...
            failure_details = "カードが利用停止されています。カード会社にお問い合わせください。"
    ...
    def get_customer_payment_history(
        ...
        for reservation, vehicle in reservations:
            if reservation.payment_reference:
                # 車両名を作成
                vehicle_name = f"{vehicle.make} {vehicle.model}" if vehicle else "車両情報なし"
                ...
                payments.append(PaymentResponse(
                    ...
                    failure_reason="カードが拒否されました" if reservation.payment_status == "failed" else None,
                    failure_code="CARD_DECLINED" if reservation.payment_status == "failed" else None,
                    failure_details="カード会社により決済が拒否されました。カードの有効性や残高をご確認ください。" if reservation.payment_status == "failed" else None,
                    # 車両情報を追加
        ...
        for reservation, vehicle, customer, store in reservations:
            if reservation.payment_reference:
                ...
                payments.append(PaymentResponse(
                    ...
                    customer_name=customer.full_name if customer else "顧客情報なし",
                    customer_email=customer.email if customer else "メールアドレスなし",
    ...
    def process_refund(
        self, payment_id: str, refund_data: RefundRequest, idempotency_key: Optional[str] = None
    ) -> RefundResponse:
        """
        返金処理を実行する
        ...
        if idempotency_key:
            ...
            existing_refund = self.db.query(Reservation).filter(
